...
_`sf::Drawable` заставляет инициализовать (`virtual void draw(RenderTarget& target, RenderStates states) const = 0;`)_ чтобы можно было вызывать `entity.draw(window)`.
